 ...
 * @file uart_linux.c
 *
 * @brief Implements the UART API for Linux Host
 *
 * %full_filespec:uart_linux.c~kok_inf#10:csrc:kok_basa#4 %
 * @version %version:kok_inf#10 %
 * @author  %derived_by:sz68t3 %
 * @date    %date_modified:Tue Sep 23 11:38:16 2014 %
 *
 *------------------------------------------------------------------------------
 *
 ...
 *
 * Implements the UART API for Linux based on posix standard terminal I/O
 *
...
UA_Channel_T UA_Open(UA_Channel_T channel, const UA_Attribute_T * attribute)
{
   ...
   Rx_Buf[channel].buffer = SAL_Alloc(UART_RX_BUF_SIZE);
...
void UA_Close(UA_Channel_T channel)
{
   ...
   {
      SAL_Free(Rx_Buf[channel].buffer);
 ...
 */
static size_t us_Read_Buf(uint8_t ** dest, UA_Rx_Buffer_T * src, size_t size)
{
   if (src->bytes < size)
   {
      size = src->bytes;
...
int32_t UA_Gets(UA_Channel_T channel, uint8_t * buf, size_t size)
{
   ...
   byte_count = us_Read_Buf(&buf, &Rx_Buf[channel], size);
 ...
 *
 * 23-Sep-2014 Ever P Cortes (sz68t3) Rev 10
 *  + Task kok_inf#12591 - Fixed a valgrind warning.
 *
 */
/*===========================================================================*/
