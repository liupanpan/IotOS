 ...
 *
 * Description: Implementation of the SAL_Purge_Only function.
 *
 * -----------------------------------------------------------------------*/
...
EM_FILENUM(XSAL_Q_MODULE_ID, 8); /* Identify file for trace/assert purposes */

/* ----------------------------
 *  Local Function Prototypes
 * ---------------------------- */

static bool Is_Message_To_Remove(SAL_Event_Id_T event_id, const SAL_Event_Id_T event_list[], size_t number_of_events);
...
static bool Is_Message_To_Remove(SAL_Event_Id_T event_id, const SAL_Event_Id_T event_list[], size_t number_of_events)
{
   ...
   size_t event_idx;
   for (event_idx = 0; event_idx < number_of_events; event_idx++)
   {
      if (event_list[event_idx] == event_id)
      {
         to_remove = true;
         break;
      }
...
void SAL_Purge_Only(const SAL_Event_Id_T event_id_list[], uint32_t number_of_events)
{
   ...
   if (SAL_Lock_Mutex(&q->queue_lock))
   {
      ...
      while (n != q->end_marker)
      {
         ...
         if (Is_Message_To_Remove(n->message.event_id, event_id_list, number_of_events))
         {
