 ...
 *
 * Description: Implementation of the SAL_Next_Message function.
 *
 * -----------------------------------------------------------------------*/
...
EM_FILENUM(XSAL_MSG2_MODULE_ID, 8); /* Identify file for trace/assert purposes */
...
void SAL_Next_Message(SAL_Queue_Iterator_T* iterator)
{
   ...
   if (SAL_Lock_Mutex(&q->queue_lock))
   {
      ...
      /** Iterator mustn't points to end of queue.
       */
      PBC_Require(*iterator != q->end_marker, "Iterator points to end of queue");
