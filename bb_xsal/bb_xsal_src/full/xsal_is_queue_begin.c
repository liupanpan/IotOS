 ...
 *
 * Description: Implementation of the SAL_Is_Queue_Begin() function.
 *
 * -----------------------------------------------------------------------*/
...
EM_FILENUM(XSAL_Q_MODULE_ID, 5); /* Identify file for trace/assert purposes */
...
bool_t SAL_Is_Queue_Begin(const SAL_Queue_Iterator_T* iterator)
{
   ...
   {
      status = (bool_t)((*iterator)->prev_node == (*iterator)->queue->end_marker);
