 ...
 *
 * Description: Implementation of the SAL_WLock_RWLock() function.
 * 
 * -----------------------------------------------------------------------*/
...
EM_FILENUM(XSAL_RWLOCK_MODULE_ID, 6); /* Identify file for trace/assert purposes */
...
bool_t SAL_WLock_RWLock(SAL_RWLock_T* rw_lock)
{
   ...
   PBC_Ensure_1(EOK == status, "SAL_WLock_RWLock error: %s", strerror(status));
   ...
   status = SAL_Wait_Semaphore(&rw_lock->write_sem);

   PBC_Ensure_1(status, "SAL_WLock_RWLock error: %d", (int)GetLastError());
