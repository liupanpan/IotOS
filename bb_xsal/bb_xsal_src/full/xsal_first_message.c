 ...
 *
 * Description: Implementation of the SAL_First_Message() function.
 *
 * -----------------------------------------------------------------------*/
...
EM_FILENUM(XSAL_MSG2_MODULE_ID, 6); /* Identify file for trace/assert purposes */
...
void SAL_First_Message(SAL_Queue_Iterator_T* iterator)
{
   ...
   if (SAL_Lock_Mutex(&q->queue_lock))
   {
      ...
      PBC_Require(q->end_marker->next_node != NULL, "NULL next pointer");

      *iterator = q->end_marker->next_node;
